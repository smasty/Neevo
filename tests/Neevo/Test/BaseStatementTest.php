...
class BaseStatementTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->stmt = new Mocks\DummyStmt(new Connection(array('driver' => 'Dummy')));
    }
    ...
    {
        $this->stmt->where($f = 'two', $v = new Mocks\DummyStmt(new Connection('driver=Dummy')));
        $this->assertEquals(
            array(
                array(
                    'simple' => true,
    ...
    {
        $this->stmt->where(
            $e = 'one = %s AND two = %i AND three = %sub',
            $a[] = 'one',
            $a[] = 2,
            $a[] = new Mocks\DummyStmt(new Connection('driver=Dummy'))
        );
        $this->assertEquals(
            array(array(
                'simple' => false,
    ...
    {
        $this->stmt->order($a = array( 'one' => '1', 'two' => '2'));
        $this->assertEquals(
            array(
                array('one', '1'),
                array('two', '2')
            ),
            $this->stmt->getSorting()
        );
    }
    ...
    {
        $r = new ReflectionProperty('Neevo\\Test\\Mocks\\DummyStmt', 'type');
    ...
    {
        $r = new ReflectionProperty('Neevo\\Test\\Mocks\\DummyStmt', 'source');
        $r->setAccessible(true);
