...
class Parser
{
    ...
    public function parse()
    {
        ...
        $type = $this->stmt->getType();
        if ($type === Manager::STMT_SELECT) {
            $q = $this->parseSelectStmt();
        } elseif ($type === Manager::STMT_INSERT) {
            $q = $this->parseInsertStmt();
        } elseif ($type === Manager::STMT_UPDATE) {
            $q = $this->parseUpdateStmt();
        } elseif ($type === Manager::STMT_DELETE) {
    ...
    protected function parseSource()
    {
        ...
        foreach ($this->stmt->getJoins() as $key => $join) {
            ...
            if ($join_source instanceof Result) {
                $join_alias = $this->escapeValue(
                    $join_source->getAlias() ? $join_source->getAlias() : '_join_' . ($key + 1),
                    Manager::IDENTIFIER
                );
